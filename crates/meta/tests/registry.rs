...
fn every_workflow_can_post_its_own_raw_log() {
    ...
    for f in files {
        ...
        assert!(
            src.contains("scripts/raw-log.sh"),
            "{f} 没接回写通道，它红了我只能看到一个比特"
        );
        assert!(
            src.contains("raw_log_outcome="),
...
fn the_formatter_terminates_by_content_and_admits_it_is_not_a_gate() {
    ...
    assert!(
        src.contains("steps.after.outputs.changed"),
