...
/// 面板至少要这么高才能工作。
///
/// 为什么是 2 而不是 1：拖到顶要到文首、拖到底要到文末，而**一个像素不可能
/// 同时是文首和文末**。这不是断言写得太严，是两个语义真的矛盾；
/// 把其中一条降成「尽量」等于把它改成装饰，而且是为了迴合一个根本没人会用的 1px 面板。
pub const MIN_PANEL_PX: u32 = 2;
...
impl JumpMap {
    /// 高度不够或行数为 0 时返回 None：那种时候面板根本不应该响应点击，
    /// 而不是假装自己能算（除零、全部映到第 0 行、或者顶底二选一）。
    pub fn new(height_px: u32, lines: usize) -> Option<Self> {
        if height_px < MIN_PANEL_PX || lines == 0 {
            return None;
        }
